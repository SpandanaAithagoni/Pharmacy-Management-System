...
class CustomerService:
    ...
    def create_customer(self, name: str, email: str, phone: str, city: str | None = None) -> Dict:
        existing = [c for c in self.dao.list_customers() if c["email"] == email]
        if existing:
    ...
    def delete_customer(self, cust_id: int) -> Dict:
        customer = self.dao.delete_customer(cust_id)
        if not customer:
            raise CustomerError("Customer not found or already deleted")
        return customer
